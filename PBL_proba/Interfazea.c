...
void renderBlackButton() {
    ...
    int buttonSize = 50;  // Tamaño del cuadrado (ajusta según sea necesario)
    int buttonX = WINDOW_WIDTH - buttonSize - 20;  // Posición en el eje X
    int buttonY = WINDOW_HEIGHT - buttonSize - 20; // Posición en el eje Y
    int cornerRadius = 31; // Radio de las esquinas redondeadas

    // Dibuja el cuadrado con esquinas redondeadas
    for (int i = 0; i < buttonSize; ++i) {
        for (int j = 0; j < buttonSize; ++j) {
            ...
            if (sqrt(dx * dx + dy * dy) < cornerRadius) {
                // Dentro del círculo: dibuja el píxel
...
void renderizarPestana() {
    ...
    SDL_SetRenderDrawColor(renderer, 100, 100, 100, 255); // Color de fondo de la pestaña
    SDL_Rect pestanaRect = { WINDOW_WIDTH - 400, 0, WINDOW_WIDTH , WINDOW_HEIGHT }; // Pestaña que ocupa la mitad de la pantalla
